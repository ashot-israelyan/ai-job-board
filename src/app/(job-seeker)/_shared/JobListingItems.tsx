...
async function SuspendedComponent({ searchParams, params }: Props) {
  ...
  if (jobListings.length === 0) {
    return <div className="text-muted-foreground p-4">No job listings found</div>;
  }
...
async function getJobListings(
  ...
  if (searchParams.jobIds) {
    whereConditions.push(or(...searchParams.jobIds.map((jobId) => eq(JobListingTable.id, jobId))));
  }
