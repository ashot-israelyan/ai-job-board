...
type Application = Pick<typeof JobListingApplicationTable.$inferSelect, 'rating'> & {
  userName: string;
  ...
  return (
    <Section className={noMargin ? undefined : 'mt-8'}>
      ...
      {Object.entries(
        applications.reduce(
          (acc, application) => {
            const jobListingId = application.jobListingId;
            if (!acc[jobListingId]) {
              acc[jobListingId] = [];
            }
            acc[jobListingId].push(application);
            return acc;
          },
          {} as Record<string, Application[]>
        )
