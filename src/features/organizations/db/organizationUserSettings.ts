...
export async function insertOrganizationUserSettings(
  ...
  await db.insert(OrganizationUserSettingsTable).values(settings).onConflictDoNothing();
...
export async function updateOrganizationUserSettings(
  ...
  settings: Partial<
    Omit<typeof OrganizationUserSettingsTable.$inferInsert, 'userId' | 'organizationId'>
  >
) {
  await db
    ...
    .onConflictDoUpdate({
      target: [OrganizationUserSettingsTable.userId, OrganizationUserSettingsTable.organizationId],
      set: settings,
    });
